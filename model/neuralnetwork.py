...
class NeuralNetwork:
    ...
    def sigmoid(self, x, option='sigmoid'):
        if option=='sigmoid':
            return 1/(1+np.exp(-(x)))
        elif option=='step':
    ...
    def run(self, X, option='standard', debug=False):
        
        # Inputs
        self.inputs = X
        contributions = self.inputs.dot(self.W['0'])
        self.hiddenLayer[0] = self.sigmoid(contributions, option=option)
        if debug:
        ...
        for l in range(1, self.nbHiddenLayer):   
            ...
            self.hiddenLayer[l] = self.sigmoid(contributions, option=option)
            if debug:
        ...
        self.outputs = self.sigmoid(contributions, option=option)
        if debug:
